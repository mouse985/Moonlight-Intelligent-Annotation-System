                ...
                with self._sam_lock:
                    if is_sam3:
                        try:
                            ...
                            logger.info(f"使用Ultralytics加载SAM3模型: {smallest_path}")
                        except Exception as e:
                            logger.warning(f"SAM3加载失败，尝试回退: {e}")
                            for d in search_dirs:
                                try:
                                    if os.path.isdir(d):
                            ...
                            if os.path.exists(p) and self._is_safe_weight(p, [
                                ...
                                with self._sam_lock:
                                    try:
                                        ...
                                        logger.info(f"预加载SAM3模型(Ultralytics): {p}")
                                    except Exception:
                                        pass
                                break
        ...
        try:
            ...
            with self._sam_lock:
                if is_sam3:
                    ...
                    self.models["sam"] = model
                    self.active_sam_type = "sam3"
                    logger.info(f"切换至SAM3模型(Ultralytics): {weight_path}")
                else:
                    from ultralytics import SAM
