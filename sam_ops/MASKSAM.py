...
class MASKSAMInferenceManager(QObject):
    """MASK模式下的SAM推理管理器，负责处理MASK模式下的SAM推理"""
    ...
    def _get_model_from_global_loader(self):
        ...
        try:
            ...
            if self.model is None:
                logger.warning("全局模型加载器中未找到SAM模型，MASK模式SAM功能将不可用")
            else:
                logger.info("从全局模型加载器成功获取SAM模型")
        except Exception as e:
            logger.error(f"从全局模型加载器获取SAM模型失败: {e}，MASK模式SAM功能将不可用")
            self.model = None
    ...
    def add_point(self, point, is_positive=True):
        ...
        # 检查模型是否已加载
        if not self.model:
            logger.warning("MASK模式SAM模型未加载，无法添加点")
            self.inference_error.emit("MASK模式SAM模型未加载，请等待模型加载完成")
            return
    ...
    def add_bbox(self, x1, y1, x2, y2):
        ...
        # 检查模型是否已加载
        if not self.model:
            logger.warning("MASK模式SAM模型未加载，无法添加BBOX")
            self.inference_error.emit("MASK模式SAM模型未加载，请等待模型加载完成")
            return
            
        # 存储BBOX坐标，确保x1,y1是左上角，x2,y2是右下角
    ...
    def start_inference(self):
        """开始MASK模式下的SAM推理"""
        if not self.model:
            logger.warning("MASK模式SAM模型未加载，无法执行推理")
            self.inference_error.emit("MASK模式SAM模型未从全局加载器获取，请检查全局模型加载状态")
            return
        ...
        if self.is_inference_running:
            logger.warning("MASK模式SAM推理正在进行中，请等待完成")
            return
    ...
    def _run_inference(self, image_path, ui_crop_info=None):
        """在后台线程中运行MASK模式下的SAM推理"""
        try:
            logger.info(f"开始MASK模式SAM推理，图片路径: {image_path}")
            
            # 保存当前输入用于推理
            inference_points = self.current_points.copy() if self.current_points else None
            inference_labels = self.current_labels.copy() if self.current_labels else None
            inference_bbox = self.current_bbox.copy() if self.current_bbox else None
            
            # 记录输入信息
            if inference_points:
                logger.info(f"输入点: {inference_points}")
                logger.info(f"输入标签: {inference_labels}")
            if inference_bbox:
                logger.info(f"输入BBOX: {inference_bbox}")
            ...
            if remote_enabled and ui_crop_info:
                try:
                    ...
                    if crop_w > 0 and crop_h > 0:
                        ...
                        if img is not None:
                            ...
                            if x1 > x0 and y1 > y0:
                                ...
                                proposed_path = os.path.join(temp_dir, f'sam_mask_crop_{ts}.png')
                                should_save = False
                    ...
                    logger.warning(f"文件级裁剪失败，回退原图: {_e}")
                    use_cropped_path = None
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(
                    f"MASK模式SAM推理结果: 图片={os.path.basename(image_path)}, 点数={len(inference_points) if inference_points else 0}, BBOX={inference_bbox}, 结果数={len(results) if results else 0}"
                )
            ...
            if self.last_dense_polygons:
                ...
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(
                        f"MASK模式致密多边形: 数量={len(self.last_dense_polygons)}, 示例点数={len(self.last_dense_polygons[0]) if len(self.last_dense_polygons) > 0 else 0}"
                    )
...
# 创建全局MASK模式SAM推理管理器实例
mask_sam_manager = None

def get_mask_sam_manager(main_window=None):
    """获取全局MASK模式SAM推理管理器实例"""
    global mask_sam_manager
    if mask_sam_manager is None:
        mask_sam_manager = MASKSAMInferenceManager(main_window)
    return mask_sam_manager
